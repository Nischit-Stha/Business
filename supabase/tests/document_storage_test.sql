...
select is((select count(*)::integer from pg_policies where schemaname='storage' and tablename='objects' and cmd in ('UPDATE','DELETE')),0,'no update or delete storage policy exists');
...
select lives_ok($$select public.register_document_upload('47000000-0000-4000-8000-000000000001','DRIVER_LICENCE','customer-documents','customers/47000000-0000-4000-8000-000000000001/driver_licence/10000000-0000-4000-8000-000000000001.pdf','licence.pdf','application/pdf',100,repeat('a',64),current_date+365)$$,'authorized upload metadata is created');
...
select throws_ok($$select public.register_document_upload('47000000-0000-4000-8000-000000000001','PROOF_OF_ADDRESS','customer-documents','customers/47000000-0000-4000-8000-000000000001/proof_of_address/10000000-0000-4000-8000-000000000002.exe','bad.exe','application/x-msdownload',100,repeat('b',64),null)$$,'22023','unsupported file type','unsupported file type rejected');
...
select throws_ok($$select public.register_document_upload('47000000-0000-4000-8000-000000000001','PROOF_OF_ADDRESS','customer-documents','../escape.pdf','escape.pdf','application/pdf',100,repeat('b',64),null)$$,'22023','invalid object metadata','path traversal rejected');
select public.register_document_upload('47000000-0000-4000-8000-000000000001','DRIVER_LICENCE','customer-documents','customers/47000000-0000-4000-8000-000000000001/driver_licence/10000000-0000-4000-8000-000000000003.pdf','replacement.pdf','application/pdf',110,repeat('c',64),current_date+400);
...
select throws_ok($$select public.register_document_upload('47000000-0000-4000-8000-000000000001','PROOF_OF_ADDRESS','customer-documents','customers/47000000-0000-4000-8000-000000000001/proof_of_address/10000000-0000-4000-8000-000000000004.pdf','address.pdf','application/pdf',100,repeat('d',64),null)$$,'42501','staff access required','unauthorized upload denied');
