...
function showAddCar() {
    const weeklyRateInput = document.getElementById('car-rate-week');
    if (weeklyRateInput && (!String(weeklyRateInput.value || '').trim() || Number(weeklyRateInput.value) <= 0)) {
        weeklyRateInput.value = String(DEFAULT_WEEKLY_RATE);
    }
    showModal('car-modal');
}
