...
export default async function FleetPage() {
  ...
  return (
    <main>
      <nav><Link href="/">Home</Link><Link href="/customers">Customers</Link><Link href="/assignments">Assignments</Link></nav>
      ...
      <div className="table-wrap"><table>
        ...
        <tbody>{vehicles.map((vehicle) => {
          ...
          return <tr key={vehicle.id}><td><strong>{vehicle.registration}</strong></td><td>{vehicle.year} {vehicle.make} {vehicle.model}</td><td><span className={`status status-${vehicle.operational_status.toLowerCase()}`}>{vehicle.operational_status.replaceAll('_', ' ')}</span></td><td>{active?.customers[0]?.full_name ?? '—'}</td><td>{movement(vehicle.operational_status)}</td></tr>;
        })}</tbody>
      </table></div>
