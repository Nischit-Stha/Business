...
export async function createNotice(form: FormData) {
  ...
  const { data, error } = await supabase.rpc('create_toll_fine_notice', {
    ...
    p_amount: Number(value(form, 'amount')), p_source: value(form, 'source'),
  });
  if (error) fail(path, error.message);
  revalidatePath(path); redirect(`/tolls-fines/${data.id}`);
}

export async function reviewNotice(form: FormData) {
  const id = value(form, 'noticeId'); const path = `/tolls-fines/${id}`; const { supabase } = await requireStaff();
  ...
  if (error) fail(path, error.message); revalidatePath(path); revalidatePath('/tolls-fines'); redirect(path);
}
