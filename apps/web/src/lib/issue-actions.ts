...
export async function notifyCustomerAboutIssue(form: FormData) { const id=value(form,'issueId'); await mutate('queue_supported_notification',{p_type:value(form,'notificationType'),p_customer_id:value(form,'customerId'),p_vehicle_id:value(form,'vehicleId'),p_agreement_id:nullable(form,'agreementId'),p_issue_id:id},`/operations/issues/${id}`); revalidatePath('/notifications'); redirect(`/operations/issues/${id}`); }
